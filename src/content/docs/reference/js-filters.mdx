...
| `isHardenedRuntime`   | Whether the process is running with App Sandbox enabled | Boolean | Yes              |
| `isSandboxed`         | Whether the process is running with App Sandbox enabled | Boolean | Yes              |
| `listeningPorts`      | List of listening ports                                                           | [Integer] | Yes              |
| `downloadBytesPerSecond` | current download bytes per second                                              | Integer   | No                |
| `uploadBytesPerSecond` | current upload bytes per second | Integer | No |
 | `totalDownloadedBytes` | total downloaded bytes since app launch |  Integer | No |
 | `totalUploadedBytes` | total uploaded bytes since app launch |  Integer | No |
