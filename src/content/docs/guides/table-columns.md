...
### Version
Shows the version of the process when available (`CFBundleShortVersionString` from `Info.plist`). If not available, tool attempts to extract the version from the executable's path.
